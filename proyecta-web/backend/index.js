...
const FRONTEND_URL = process.env.FRONTEND_URL || process.env.PUBLIC_APP_URL || "https://proyecta-mining.pages.dev"
...
function isAllowedOrigin(origin) {
  ...
  return /^https:\/\/([a-z0-9-]+\.)?proyecta(-mining)?\.pages\.dev$/i.test(
    origin,
  )
}
...
function buildPasswordResetLink(token) {
  ...
  return `${base}/restablecer-contraseÃ±a?token=${encodeURIComponent(token)}`
}
...
async function sendPasswordResetEmail({ user, resetLink }) {
  ...
  const subject = "Recupera tu contraseÃ±a de DivulgarÃƒÂ­a"
  const text = [
    `Hola, ${user.name}.`,
    "",
    "Recibimos una solicitud para restablecer tu contraseÃ±a en DivulgarÃƒÂ­a.",
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#14213d;max-width:640px;margin:0 auto;padding:24px">
      ...
      <h1 style="font-size:28px;line-height:1.2;margin:0 0 16px">Recupera tu contraseÃ±a</h1>
...
function mapStoredReview(review, reviewer, store) {
  ...
  const rating = normalizeReviewScore(
    review.rating,
    (clarity + rigor + utility + novelty + reproducibility) / 5,
  )
...
function buildOrcidAuthorizeUrl(state) {
  ...
  return (
    `${ORCID_BASE_URL}/oauth/authorize?client_id=${clientId}` +
    `&response_type=code` +
    `&scope=/authenticate` +
  ...
  try {
    ...
    const works = workGroups
      .map((group) => {
        ...
        const doiId = externalIds.find((item) => String(item["external-id-type"] || "").toLowerCase() === "doi")
...
async function fetchOpenAlexAuthorMetrics(orcid) {
  ...
  try {
    ...
    const firstInstitution = safeArray(author.last_known_institutions)[0]
    const primaryInstitution =
      firstInstitution?.display_name || firstInstitution?.institution?.display_name || ""
...
app.get("/api/meta/community", async (_req, res) => {
  ...
  res.json({
    ...
    novas: {
      ...
      targetNetwork: "Base",
      summary:
        "DIVULS incentiva publicacin, revisin y divulgaciÃƒÂ³n dentro de DivulgarÃƒÂ­a. En esta fase funciona como saldo interno, listo para una futura migraciÃ³n a Base.",
      policy: {
        publicationReward: 25,
        reviewReward: 15,
        commentReward: 5,
...
app.post("/api/password/forgot", async (req, res) => {
  ...
  if (!passwordResetMailer && !allowPasswordResetPreview(req)) {
    return res.status(503).json({
      error:
        "La recuperaciÃ³n por correo a?n no esta configurada en este entorno pÃºblico. Podemos dejarla activa en cuanto conectemos un servicio SMTP.",
    })
  }
  ...
  return res.json({
    ok: true,
    message:
      "Si el correo existe en DivulgarÃƒÂ­a, enviaremos un enlace temporal para restablecer la contraseÃ±a.",
    previewResetLink,
  })
})
...
app.get("/api/password/reset/validate", async (req, res) => {
  ...
  if (!token) {
    return res.status(400).json({ error: "El token de recuperaciÃ³n es obligatorio." })
  }
  ...
  if (!resetRequest) {
    await writeStore(store)
    return res.status(400).json({ error: "El enlace de recuperaciÃ³n ya no es valido o ha expirado." })
  }
...
app.post("/api/password/reset", async (req, res) => {
  ...
  if (!token || !password.trim()) {
    return res.status(400).json({ error: "El token y la nueva contraseÃ±a son obligatorios." })
  }
  ...
  return res.json({
    ok: true,
    message: "Tu contraseÃ±a ya fue actualizada. Ahora puedes iniciar sesion con la nueva clave.",
  })
})
...
app.patch("/api/me", async (req, res) => {
  ...
  auth.user.name = name
  auth.user.affiliation = affiliation || "Comunidad DivulgarÃƒÂ­a"
  auth.user.bio = bio || "Perfil cientÃƒÂ­fico en crecimiento dentro de DivulgarÃƒÂ­a."
...
app.get("/api/articles", async (req, res) => {
  ...
  const articles = store.posts
    ...
    .map((post) => {
      ...
      article.viewerState = {
        vote: viewerUserId ? Number(store.votes.find((item) => item.articleId === post.id && item.userId === viewerUserId)?.value || 0) : 0,
        followingAuthor:
          !!viewerUserId &&
          !!author &&
          store.follows.some(
...
app.get("/api/community/overview", async (_req, res) => {
  ...
  const recentReviews = store.reviews
    ...
    .map((review) => {
      ...
      return {
        id: review.id,
        reviewer: reviewer.name || "Miembro DivulgarÃƒÂ­a",
  ...
  const recentComments = store.comments
    ...
    .map((comment) => {
      ...
      return {
        id: comment.id,
        author: commenter.name || "Miembro DivulgarÃƒÂ­a",
        comment: comment.comment,
...
app.get("/api/articles/:id", async (req, res) => {
  ...
  if (!post) {
      return res.status(404).json({ error: "ArtÃƒÂ­culo no encontrado." })
  }
  ...
  return res.json({
    ...
    viewerState: {
      vote: viewerVote ? Number(viewerVote.value || 0) : 0,
      review: viewerReview
        ? {
...
async function handleOrcidCallback(req, res) {
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      ...
      return res.redirect(`${FRONTEND_URL}/login?oauth_error=token_exchange`)
    }

    const orcid = tokenData.orcid
    const accessToken = tokenData.access_token
    if (!orcid) {
      return res.redirect(`${FRONTEND_URL}/login?oauth_error=no_orcid`)
    }
    ...
    if (oauthState.mode === "link" && oauthState.userId) {
      ...
      if (!linkedUser) {
        return res.redirect(`${FRONTEND_URL}/login?oauth_error=link_user_missing`)
      }
    ...
    return res.redirect(`${FRONTEND_URL}/login?oauth_error=callback`)
  }
}
