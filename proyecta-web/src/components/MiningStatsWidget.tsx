...
export function MiningStatsWidget({ wallet, fundingGoal, projectTitle, projectId }: MiningStatsWidgetProps) {
  ...
  const fetchMiningStats = async () => {
    try {
      ...
      setStats(null)
      setError(err instanceof Error ? err.message : 'Error al obtener estad铆sticas')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="nova-card space-y-3 bg-gradient-to-br from-blue-50 to-purple-50 p-6">
        <div className="flex items-center justify-between">
          <h3 className="flex items-center gap-2 font-bold text-slate-900">
            ...
            Miner铆a comunitaria en progreso
          </h3>
          <div className="inline-flex h-5 w-5 animate-spin rounded-full border-2 border-purple-600 border-t-transparent" />
        </div>
        <p className="text-sm text-slate-600">Consultando el saldo visible del portal y la confirmaci贸n del pool...</p>
      </div>
    )
  }
      ...
      <div className="nova-card space-y-4 border-2 border-slate-200 bg-white p-6">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h3 className="flex items-center gap-2 text-lg font-bold text-slate-900">
              ...
              Miner韆 comunitaria lista
            </h3>
            <p className="mt-1 text-sm leading-6 text-slate-600">
              A鷑 no se ha iniciado miner韆 para <span className="font-semibold">{projectTitle}</span>. No se muestra el historial global de la wallet como avance de este proyecto.
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="font-bold text-slate-900">Sin miner韆 iniciada</p>
          </div>
        </div>

        <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm leading-6 text-blue-900">
          La direcci髇 del investigador sigue siendo v醠ida para recibir XMR, pero el progreso de este proyecto empieza en cero hasta que alguien elija navegador o app y aporte c髆puto.
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="nova-card space-y-4 border-2 border-amber-200 bg-amber-50 p-6">
        ...
        <div className="space-y-2 rounded-lg border border-amber-200 bg-white p-4 text-sm text-amber-900">
          <p className="font-bold">Esperando confirmaci贸n del pool</p>
          <p>
            No pudimos leer el resumen de miner铆a todav铆a. Si la prueba local est谩 activa, el navegador puede seguir aportando mientras el pool confirma los datos.
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="nova-card space-y-4 border-2 border-purple-200 bg-gradient-to-br from-purple-50 to-blue-50 p-6">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            {confirmed && localActive
              ...
              : confirmed
                ? 'SupportXMR confirm贸 la direcci贸n; el saldo visible sigue sumando el aporte local.'
                : localActive
                  ? 'Aporte local visible del navegador; el saldo visible del portal suma ese aporte mientras llega la confirmaci贸n.'
                  : 'Esperando confirmaci贸n del pool. El saldo visible puede incluir prueba local.'}
          </p>
        </div>
        <button
          onClick={fetchMiningStats}
      ...
      <div className={`inline-flex rounded-full border px-3 py-1 text-xs font-bold ${confirmed ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : localActive ? 'border-amber-200 bg-amber-50 text-amber-800' : 'border-slate-200 bg-slate-50 text-slate-700'}`}>
        {confirmed && localActive ? 'Confirmado + local visible' : confirmed ? 'Saldo confirmado por SupportXMR' : localActive ? 'Aporte local visible' : 'Esperando confirmaci贸n del pool'}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          ...
          <p className="mt-1 text-xs text-slate-500">{confirmedInvalidShares.toLocaleString('es-ES')} inv谩lidos</p>
        </div>
      ...
      <div className="space-y-2 rounded-lg border border-slate-200 bg-white p-4">
        ...
        <p className="text-xs text-slate-600">
          Direcci贸n: <code className="break-all rounded bg-slate-100 px-2 py-1 font-mono text-xs">{wallet.substring(0, 32)}...</code>
        </p>
        <p className="text-xs text-slate-600">
      ...
      <div className="rounded-lg border-2 border-emerald-300 bg-gradient-to-r from-emerald-50 to-cyan-50 p-4">
        <p className="mb-2 text-xs font-bold text-emerald-900">C贸mo funciona la miner铆a comunitaria</p>
        <ul className="space-y-1 text-xs text-emerald-800">
          <li>Comunidad elige iniciar miner铆a para este proyecto</li>
          <li>Cada participante aporta poder de c贸mputo (App o Navegador)</li>
          <li>SupportXMR acumula hashes y paga en XMR autom谩ticamente</li>
          <li>XMR va directamente a la direcci贸n del investigador</li>
