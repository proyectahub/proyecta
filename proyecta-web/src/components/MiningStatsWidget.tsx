...
export function MiningStatsWidget({ wallet, fundingGoal, projectTitle, projectId }: MiningStatsWidgetProps) {
  ...
  const fetchMiningStats = async () => {
    try {
      ...
      setError(projectId ? 'No fue posible consultar las estad韘ticas del proyecto.' : 'No fue posible consultar SupportXMR en este momento.')
    } catch (err) {
      setStats(projectId ? createEmptyProjectStats() : null)
      setError(err instanceof Error ? err.message : 'Error al obtener estad韘ticas')
    } finally {
      setLoading(false)
    }
  }
