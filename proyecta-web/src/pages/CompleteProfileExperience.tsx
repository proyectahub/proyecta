...
export function CompleteProfileExperience() {
  ...
  const { user, logout, updateProfile } = useWalletAuth()
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-purple-50 p-4">
        <div className="w-full max-w-lg space-y-6">
          ...
          <div className="nova-card space-y-6 p-8">
            ...
            <div className="flex gap-3"><button onClick={logout} className="nova-button-soft flex-1 text-red-600">Desconectar</button><button onClick={() => void handleSaveInfo()} disabled={!fullName.trim()} className="nova-button-solid flex-1 disabled:opacity-50">Continuar</button></div>
          </div>
        </div>
      </div>
    )
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-purple-50 p-4">
        <div className="w-full max-w-lg space-y-6">
          ...
          <div className="nova-card space-y-6 p-8">
            ...
            <div className="flex gap-3"><button onClick={() => setStep('info')} className="nova-button-soft flex-1">Volver</button><button onClick={() => void handleAddOrcid()} className="nova-button-solid flex-1">Completar</button></div>
          </div>
        </div>
      </div>
    )
