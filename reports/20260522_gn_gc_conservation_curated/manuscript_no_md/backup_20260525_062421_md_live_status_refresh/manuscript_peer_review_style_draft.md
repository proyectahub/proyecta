...
- Molecular dynamics evidence is now broader but still partial for a fully balanced two-receptor control frame: completed trajectories are available for 9P3Y-granisetron, 5E04-paroxetine repeat, 5E04-ribavirin control, 9P3Y-NAG structural control, and a remotely completed 5E04-apo trajectory recorded in the cached queue snapshot dated 2026-05-25. However, the 5E04-apo files have not yet been imported into the local analysis bundle used here, and 9P3Y-apo remains only in partial setup.
...
Molecular dynamics was used as a post-docking structural-consistency layer for branch-selected systems when trajectories were available. At the current manuscript snapshot, the two branch representatives have analyzable trajectories through 43.25 ns for 5E04-paroxetine repeat and 50.00 ns for 9P3Y-granisetron. These descriptors are reported as computational stability indicators only. They are not interpreted as binding free energies, antiviral activity, or experimental validation. Completed comparator trajectories now exist for 5E04-ribavirin and 9P3Y-NAG. In addition, a cached remote queue snapshot dated 2026-05-25 indicates that the 5E04-apo production job has completed remotely, but no local apo-analysis export was available in the present package; 9P3Y-apo remains outside completed production and therefore the receptor-only control frame is still analytically incomplete here.

### Figure 7. Candidate-trajectory backbone, compactness, thermodynamic, and flexibility diagnostics
![Candidate-trajectory backbone, compactness, thermodynamic, and flexibility diagnostics](composite_figures/Figure_7_md_basic_candidate_diagnostics.png)

Figure 7. Basic molecular-dynamics diagnostics for the two branch representatives. The composite summarizes backbone RMSD, potential energy, radius of gyration, and residue-level RMSF for 5E04-paroxetine repeat and 9P3Y-granisetron. Across the available windows, both systems remain within bounded protein-level structural fluctuations, whereas ligand-coupled interpretation remains conservative and receptor-specific. These trajectories support post-docking structural review only and do not establish experimental affinity or antiviral activity.

### Figure 8. Protein-ligand proximity and interaction diagnostics from available candidate trajectories
![Protein-ligand proximity and interaction diagnostics from available candidate trajectories](composite_figures/Figure_8_md_interaction_proximity_panels.png)

Figure 8. Advanced molecular-dynamics diagnostics for the current candidate trajectories. Contact-count and protein-ligand center-of-mass distance traces are shown for both branch representatives. For 5E04-paroxetine repeat, the available exported bundle additionally supports a residue-resolved contact heatmap and an interaction-profile summary, highlighting recurrent contributions from residues such as ALA310, PRO311, SER332, SER142, ARG199, GLN335, SER217, ASP312, PHE360, TYR364, GLY145, and PHE307. The residue-level contact layer has now been extended to the additional completed ligand trajectories used as comparators or structural controls, including 9P3Y-granisetron, 5E04-ribavirin, and 9P3Y-NAG. For those added systems, residue-level panels are derived from a conservative geometric fallback based on per-residue minimum distance <= 0.35 nm rather than from a richer interaction-typing backend, so they should be interpreted strictly as proximity descriptors. All contact-like metrics are treated as geometric or topology-dependent computational descriptors only.
...
Taken together, the available candidate trajectories support a conservative two-branch interpretation rather than a single unified mechanistic claim. The 5E04-paroxetine repeat system shows low backbone RMSD and narrow radius-of-gyration variation, while the ligand-fitted RMSD distribution is broad and the protein-ligand center-of-mass distance remains relatively large in the exported trace. This pattern is more consistent with structural-sampling heterogeneity than with a claim of persistent locked binding. The 9P3Y-granisetron trajectory shows a similarly bounded protein-level fluctuation regime, with sustained contact counts and shorter protein-ligand COM distance than the paroxetine repeat system in the currently exported bundle; however, this remains an exploratory glycoprotein-surface-pocket hypothesis and does not establish functional inhibition or experimental binding.

The molecular-dynamics layer therefore closes the paper only at the level of computational consistency. It supports retention of both receptor-branch representatives for future non-computational evaluation, but it does not resolve whether the observed dynamics are ligand-specific rather than receptor-intrinsic until the 5E04 apo trajectory is imported and analyzed locally and the 9P3Y apo-control frame is completed.
...
| 5E04-apo-protein | receptor-only control | remotely completed in cached queue snapshot (`latest_mtime` 2026-05-25 03:16:30), but local analysis export not yet imported | needed to separate ligand-associated behavior from receptor-intrinsic fluctuations in 5E04 before receptor-only comparison can be analyzed in the publication bundle |
| 9P3Y-apo-protein | receptor-only control | partial setup only in cached queue snapshot (`latest_mtime` 2026-05-25 03:16:42; no production artifacts yet) | needed to complete receptor-only baseline for 9P3Y branch comparison |
...
The current molecular-dynamics publication bundle, including per-system PNG, CSV, histogram panels, summary statistics, and conservative auto-interpretation, is archived in `md_report/publication_report_20260525_034735/`. The cached queue-status refresh used for the present manuscript update is archived separately in `md_report/status_refresh_20260525_042149/`.
