...
- Molecular dynamics evidence is now broader but still partial for a fully balanced two-receptor control frame: completed trajectories are available for 9P3Y-granisetron, 5E04-paroxetine repeat, 5E04-ribavirin control, 9P3Y-NAG structural control, and a running 5E04-apo trajectory; 9P3Y-apo remains in pre-production setup.
...
The molecular-dynamics layer therefore closes the paper only at the level of computational consistency. It supports retention of both receptor-branch representatives for future non-computational evaluation, but it does not resolve whether the observed dynamics are ligand-specific rather than receptor-intrinsic until the apo-control frame is completed.
...
| 5E04-apo-protein | receptor-only control | running (trajectory and `md.log` advancing; queue check 2026-05-25) | needed to separate ligand-associated behavior from receptor-intrinsic fluctuations in 5E04 |
| 9P3Y-apo-protein | receptor-only control | setup/queued (no `md.*` trajectory files yet at queue check 2026-05-25) | needed to complete receptor-only baseline for 9P3Y branch comparison |
...
The current molecular-dynamics publication bundle, including per-system PNG, CSV, histogram panels, summary statistics, and conservative auto-interpretation, is archived in `md_report/publication_report_20260525_031252/`.
