...
## Abstract
A reproducible computational drug-repurposing workflow was applied to two Andes orthohantavirus structural targets: the nucleoprotein structure PDB 5E04 and the Gn/Gc glycoprotein-complex structure PDB 9P3Y. Approved-source compounds from ChEMBL and DrugCentral were standardized, deduplicated, filtered with RDKit-based physicochemical and medicinal-chemistry criteria, prioritized using transparent heuristic QSAR/ADMET scoring, and docked with AutoDock Vina. The final archived screening run assembled 4,986 source records, collapsed them to 3,460 unique ligands, retained 934 after preprocessing, and docked 93 prioritized compounds. Target-level context was assessed before docking: 5E04 was characterized using receptor-intrinsic structural parameters, whereas 9P3Y was contextualized through Gn/Gc conservation and regional alignment analyses across representative orthohantaviruses. The strongest global Vina score was obtained for paroxetine against 5E04 (-8.730 kcal/mol), while the 9P3Y receptor-specific ranking prioritized granisetron for a glycoprotein-associated exploratory pocket (-6.523 kcal/mol). Higher-exhaustiveness redocking, Vinardo rescoring, PyMOL pose inspection, PLIP/proximity annotation, rule-based ADMET profiling, chemical-diversity analysis, and molecular-dynamics descriptors were integrated as conservative structural-consistency layers. Fresh molecular dynamics showed 100 ns trajectories for 5E04-paroxetine, 5E04 apo-protein, 9P3Y-granisetron, and 9P3Y apo-protein. Paroxetine showed receptor-level behavior similar to the 5E04 apo control, whereas granisetron showed a more dynamic 9P3Y trajectory with a richer PLIP interaction-type profile. These results support paroxetine and granisetron as computationally prioritized, experimentally testable repurposing hypotheses. They do not establish antiviral activity, binding affinity, safety, or therapeutic efficacy.
...
Molecular dynamics was used as an orthogonal computational follow-up for branch-representative docked complexes and receptor-only controls where available. Fresh production trajectories from the 20260531-065046 job family were used for the final MD interpretation. The completed 100 ns systems were 5E04-paroxetine, 5E04 apo-protein, 9P3Y-granisetron, and 9P3Y apo-protein. Structural diagnostics included backbone RMSD, mean-centered potential energy, radius of gyration, and residue-level RMSF. Ligand-bearing systems were further analyzed for protein-ligand contact counts, center-of-mass distance, hydrogen-bond counts where topology-compatible, geometric residue-time contacts using a 0.35 nm heavy-atom cutoff, and PLIP interaction-type profiles. Potential-energy values were interpreted only within a given simulated system because absolute potential energy scales with system size, solvent, ion content, and topology, and the plotted panel is centered on each system mean for visual comparability only. All MD descriptors are interpreted as computational structural-consistency metrics only; they do not establish binding affinity, antiviral activity, or clinical relevance.
...
Figure 7. Receptor-separated structural diagnostics for the analyzable molecular-dynamics systems using receptor-only controls where available. All panels were regenerated from the fresh 20260531-065046 job family. The 5E04-paroxetine, 5E04 apo-protein, 9P3Y-granisetron, and 9P3Y apo-protein runs reached 100 ns. Backbone RMSD, mean-centered potential energy, radius of gyration, and residue-level RMSF are used as receptor-level structural diagnostics only. The potential-energy panels are centered on each system mean to improve visual comparability; absolute potential-energy values are not compared across receptor systems because they scale with system size, solvent, ion content, and topology.

In the 5E04 branch, the paroxetine-bearing trajectory remained close to the receptor-only apo control over 100 ns. Backbone RMSD ended at 0.232 nm for 5E04-paroxetine and 0.235 nm for 5E04 apo, with similar compactness profiles (final Rg 2.077 nm and 2.051 nm, respectively). This supports a conservative interpretation that the docked paroxetine pose is compatible with the nucleoprotein model without causing large receptor-level destabilization. It does not prove ligand-specific stabilization, affinity, or antiviral activity.

In the 9P3Y branch, the granisetron-bearing trajectory remained more mobile than the apo-protein control across the full 100 ns window. Backbone RMSD ended at 0.699 nm for 9P3Y-granisetron at 100 ns, whereas 9P3Y apo reached 0.458 nm at 100 ns. Radius of gyration also separated between the ligand-bearing and apo trajectories, with 9P3Y-granisetron ending near 2.917 nm and apo ending near 2.759 nm. The higher RMSD/RMSF values for the glycoprotein-associated branch are consistent with greater intrinsic mobility and/or ligand-associated local rearrangement in the 9P3Y model.

### Figure 8. Protein-ligand proximity and bond-like diagnostics
...
Figure 8. Protein-ligand proximity and bond-like diagnostics for the completed ligand-bearing fresh trajectories. The panel shows total protein-ligand contacts below 0.35 nm, protein-ligand center-of-mass distance, and hydrogen-bond counts for 5E04-paroxetine and 9P3Y-granisetron over completed 0-100 ns fresh trajectories. These observables are interpreted as topology- and geometry-dependent computational descriptors only.

The paroxetine trajectory showed intermittent contact episodes and relatively sparse hydrogen-bond events. Its contact trace included strong contact bursts, but hydrogen bonds remained infrequent (mean approximately 0.025; final 0). Its protein-ligand COM distance fluctuated over a broad range (mean approximately 4.22 nm), which is consistent with a surface or peripheral-pocket geometry where center-of-mass distance is not equivalent to local contact retention. This pattern supports conservative pose-compatibility interpretation rather than a claim of persistent locked binding.

The granisetron trajectory showed more frequent bond-like interaction events than paroxetine. Contact counts were sustained across broad portions of the trajectory (mean approximately 30.94 contacts), and the hydrogen-bond trace showed recurrent events (mean approximately 0.321; final 0). The COM distance averaged approximately 2.90 nm and ended near 2.50 nm, indicating recurrent proximity despite receptor-level mobility. These patterns are compatible with a more chemically diverse interaction mode in the 9P3Y branch, but they do not validate entry inhibition or binding affinity.

### Figure 9. Residue-resolved contact and PLIP interaction-type profiles
![Residue-resolved contact panels for the fresh 5E04-paroxetine and 9P3Y-granisetron dynamics](composite_figures/Figure_9_contact_panels_PAROXETINE_GRANISETRON_updated.png)

Figure 9. Residue-resolved contact panels for the fresh 5E04-paroxetine and 9P3Y-granisetron 100 ns trajectories. Each panel combines a total protein-ligand contact trace, a residue-time geometric-contact heatmap recalculated using a 0.35 nm heavy-atom cutoff, and a PLIP interaction-type profile. The PLIP profile distinguishes H-bonds, hydrophobic contacts, ionic contacts, water bridges, pi-stacking, pi-cation, and related classes when detected.

Paroxetine contacts were dominated by hydrophobic interactions, with fewer water-bridge, hydrogen-bond, and pi-stacking events. The typed PLIP summary contained 38 hydrophobic records, 4 water-bridge records, 3 hydrogen-bond records, and 1 pi-stacking record. Residue-time contacts were intermittent and concentrated in defined windows, supporting the interpretation of a structurally plausible but not continuously locked pose.

Granisetron showed a broader interaction-type profile across the 100 ns trajectory. The typed PLIP summary contained 168 hydrophobic records, 85 hydrogen-bond records, 23 pi-stacking records, 13 water-bridge records, 7 pi-cation records, and 4 ionic records. This richer interaction profile supports the 9P3Y branch as a chemically distinct exploratory hypothesis. The same trajectory also showed higher receptor-level RMSD/RMSF, so the appropriate interpretation is not that granisetron is validated as a stable inhibitor, but rather that it maintains recurrent, chemically diverse contacts within a more mobile glycoprotein-associated structural model.

**Table 6. Final molecular-dynamics interpretation summary.**

| branch | system | status | structural interpretation | interaction interpretation | interpretation boundary |
| --- | --- | --- | --- | --- | --- |
| 5E04 nucleoprotein | paroxetine | fresh 100 ns complete | RMSD and Rg are close to 5E04 apo, supporting receptor-level compatibility | mostly hydrophobic and intermittent contacts with sparse H-bonds | computational pose-consistency only; no affinity or activity claim |
| 5E04 nucleoprotein | apo-protein | fresh 100 ns complete | receptor-only baseline shows bounded intrinsic motion | not applicable | supports interpretation of paroxetine relative to receptor-intrinsic behavior |
| 9P3Y glycoprotein | granisetron | fresh 100 ns complete | higher RMSD/RMSF than apo-protein control, consistent with a more mobile glycoprotein-associated model | chemically diverse PLIP profile including H-bonds, hydrophobic contacts, pi-stacking, pi-cation, water bridges, and ionic contacts | exploratory glycoprotein-pocket hypothesis; no entry-inhibition or affinity claim |
| 9P3Y glycoprotein | apo-protein | fresh 100 ns complete | receptor-only baseline shows bounded intrinsic motion | not applicable | supports interpretation of granisetron relative to receptor-intrinsic behavior |

Taken together, the MD layer supports retention of paroxetine and granisetron as branch-representative, experimentally testable computational hypotheses. It does not justify expanding the main MD story to every screened compound. Additional screened ligands remain important as docking and prioritization comparators, but the final MD interpretation is intentionally restricted to the two receptor-branch representatives and receptor-only controls where available.

## Discussion
This study provides an integrated computational framework for Andes orthohantavirus repurposing hypotheses. The workflow combined approved-source ligand provenance, explicit ligand curation, receptor-specific docking, redocking/rescoring robustness checks, residue-level interaction annotation, PyMOL pose inspection, chemical-diversity analysis, rule-based ADMET/QSAR context, Gn/Gc conservation analysis, and molecular-dynamics follow-up for branch representatives. The strongest global prioritization signal centered on 5E04-paroxetine, while a separate receptor-specific 9P3Y-granisetron branch was retained to avoid losing a glycoprotein-associated hypothesis within the stronger 5E04 score distribution.

The most defensible interpretation is a two-branch hypothesis-generating framework. Paroxetine is the stronger global docking/prioritization candidate and shows the cleaner receptor-level MD comparison because the 5E04 apo control is complete and similar in RMSD/Rg behavior. Granisetron retains value as an exploratory glycoprotein-associated hypothesis because it shows a more diverse PLIP interaction profile over 100 ns, including hydrogen bonds, hydrophobic contacts, pi-stacking, pi-cation, water bridges, and ionic contacts, while its matched 9P3Y apo control is now complete over the same window.

The potential-energy panels require conservative interpretation. Absolute potential-energy values differ substantially between 5E04 and 9P3Y because potential energy is an extensive property that depends on the full simulated system, including receptor size, solvent, ion content, and topology. The figure therefore centers each trajectory on its own mean to make the fluctuation thickness visually comparable. The useful observation is that each trajectory remains within a bounded energy band without gross discontinuity, supporting numerical stability of the simulations.

From a manuscript-completion perspective, the current MD set is sufficient to support the computational conclusions presented here. Running additional 100 ns MD simulations for other docked compounds would substantially expand the comparison space and invite additional requirements for matched controls, replicate simulations, and equivalent downstream contact analyses. The screened compounds should therefore remain as docking, chemical-diversity, and prioritization comparators rather than being promoted into the main MD-supported candidate set. This keeps the manuscript focused on two experimentally testable hypotheses rather than diluting the narrative across many partially characterized ligands.

In vitro evaluation would be reasonable as an exploratory next step. Paroxetine would be the higher-priority candidate for an initial nucleoprotein-branch test because it combines the strongest global computational prioritization with receptor-level MD behavior similar to 5E04 apo. Granisetron would be a secondary but still testable glycoprotein-branch hypothesis because its 9P3Y trajectory is structurally more dynamic but chemically richer at the interaction-profile level. Both should be presented as computationally prioritized hypotheses, not as validated antiviral hits.

## Limitations
- Docking scores are scoring-function outputs and are not experimental affinities or binding free energies.
- Receptors were treated with limited flexibility during docking and do not represent all biological conformational states.
- The 9P3Y glycan/NAG-associated pocket is exploratory and should not be interpreted as a validated fusion-loop, receptor-binding, or entry-inhibition site.
...
- Protonation, tautomeric state, stereochemistry, receptor preparation, and ligand preparation assumptions can alter docking and interaction results.
- Potential-energy values are not comparable across different receptor systems because they scale with system size, solvent, ions, and topology.
- Molecular dynamics was performed for branch representatives rather than all screened compounds.
- The 9P3Y apo-protein control is included as a completed 100 ns receptor-only baseline.
- PLIP interaction typing is frame-sampled and topology/geometry dependent; it supports structural annotation but not biochemical validation.
- No biochemical, biophysical, cellular, animal, or clinical validation is included.

## Conclusions
This study prioritizes two receptor-specific computational repurposing hypotheses against Andes orthohantavirus structural targets. Paroxetine was prioritized for the 5E04 nucleoprotein branch and showed 100 ns MD behavior that remained broadly comparable to the 5E04 apo-protein control. Granisetron was prioritized for the 9P3Y glycoprotein branch and showed a more dynamic receptor-level trajectory but a richer PLIP interaction-type profile over 100 ns. The combined docking, redocking, rescoring, interaction, conservation, and MD layers support a focused shortlist for downstream experimental review. The appropriate next step is exploratory in vitro testing of paroxetine and granisetron under suitable assay and biosafety conditions, while retaining the other screened compounds as computational comparators rather than expanding the main MD narrative.

## Data and supplementary material
The main analysis includes receptor-specific PyMOL docking figures, composite analysis figures, docking/prioritization tables, and molecular-dynamics summaries. Complete machine-readable tables are provided as supplementary material, including selected GPC sequences, pairwise identity, regional conservation summaries, receptor rankings, Vina docking results, pharmacological plausibility tables, docking validation summaries, docking-box sensitivity outputs, consensus rankings, ADMET profiles, and risk-flag summaries. The final MD figure set and document build are archived in `md_report/final_docx_100ns_20260602/`, and the final composite figures are stored in `composite_figures/`.
