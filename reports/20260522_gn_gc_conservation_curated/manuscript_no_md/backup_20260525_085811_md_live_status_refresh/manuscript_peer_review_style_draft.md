...
- Molecular dynamics evidence is now broader but still partial for a fully balanced two-receptor control frame: completed trajectories are available for 9P3Y-granisetron, 5E04-paroxetine repeat, 5E04-ribavirin control, and 9P3Y-NAG structural control. Direct SSH inspection on 2026-05-25 showed that the 5E04-apo production run had advanced to 46.326 ns and had generated remote protein-only analysis files, but the trajectory was still running and was therefore not used as a completed receptor-only baseline here; 9P3Y-apo remained queued with no production artifacts.
...
Molecular dynamics was used as a post-docking structural-consistency layer for branch-selected systems when trajectories were available. At the current manuscript snapshot, the two branch representatives have analyzable trajectories through 43.25 ns for 5E04-paroxetine repeat and 50.00 ns for 9P3Y-granisetron. These descriptors are reported as computational stability indicators only. They are not interpreted as binding free energies, antiviral activity, or experimental validation. Completed comparator trajectories now exist for 5E04-ribavirin and 9P3Y-NAG. Live SSH inspection on 2026-05-25 further showed that the 5E04-apo production job had already exposed remote protein-only analysis outputs through approximately 46.7 ns (RMSD, RMSF, Rg, SASA, density, pressure, temperature, and potential-energy series), and those archived `.xvg` files are now used here as a provisional receptor-only structural control for the 5E04 branch. The matched 9P3Y-apo job remained queued with no production artifacts, so the receptor-only control frame is still incomplete for that branch.

### Figure 7. Receptor-separated structural diagnostics with ligand-bearing and receptor/control trajectories
![Receptor-separated structural diagnostics with ligand-bearing and receptor/control trajectories](composite_figures/Figure_7_md_structural_controls.png)

Figure 7. Receptor-separated structural diagnostics for the currently analyzable molecular-dynamics systems. The 5E04 column now compares the branch representative (paroxetine), the ligand-bearing computational comparator (ribavirin), and the archived receptor-only apo-protein control extracted from the live SSH refresh through approximately 46.7 ns. The 9P3Y column compares the branch representative (granisetron) with the completed NAG structural control; the matched 9P3Y apo-protein run remained queued at this manuscript cut and therefore cannot yet be plotted as a receptor-only baseline. Across these currently available windows, the plotted descriptors are used only to contextualize receptor-level structural fluctuations and branch-specific comparators. They do not establish ligand-specific stabilization, experimental affinity, or antiviral activity.

### Figure 8. Protein-ligand proximity and bond-like diagnostics shown separately from structural controls
![Protein-ligand proximity and bond-like diagnostics shown separately from structural controls](composite_figures/Figure_8_md_proximity_bonds_controls.png)

Figure 8. Protein-ligand proximity and bond-like diagnostics are shown separately from the receptor-level structural controls to avoid conflating apo-protein behavior with ligand-contact observables. The 5E04 column compares paroxetine with the ribavirin computational comparator for contact-count and hydrogen-bond-like traces; a matched ribavirin COM series was not available in the local export bundle and is therefore left explicit as missing. The 9P3Y column compares granisetron with the completed NAG structural control for contact-count and center-of-mass distance traces, while hydrogen-bond-like series are currently available only for granisetron. The currently archived NAG proximity export covers only an early sub-window rather than the full 50 ns production trajectory and should therefore be interpreted as a partial control trace. All contact-like metrics remain computational geometry/topology descriptors only.

In practical interpretive terms, the new apo/control framing changes the molecular-dynamics narrative in a conservative but useful way. For the 5E04 branch, the archived apo-protein traces show that the receptor itself samples bounded structural fluctuations even without ligand, which limits how strongly the paroxetine repeat can be described as ligand-specific stabilization on the basis of RMSD, Rg, or RMSF alone. The ribavirin control adds a branch-matched ligand-bearing comparator, but the currently available bundle still supports only a partial proximity layer for that system. For the 9P3Y branch, the absence of a completed apo-protein run means that receptor-intrinsic and ligand-associated components still cannot be fully separated; however, the finished NAG control and the granisetron trajectory together provide a receptor-matched control frame that is more informative than a candidate-only view.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 5E04 nucleoprotein | paroxetine repeat | 43.25 ns | 0.255 +/- 0.034 | 2.300 +/- 3.979 | 2.069 +/- 0.012 | -1271416.943 +/- 1343.916 | 985.536 +/- 0.380 | 16.0 | 5.539 | 0.0 | protein-level fluctuations remain bounded in the sampled window, whereas ligand-motion episodes and long protein-ligand COM distance argue for conservative geometric interpretation only |
| 9P3Y glycoprotein | granisetron | 50.00 ns | 0.432 +/- 0.064 | 1.337 +/- 2.422 | 2.888 +/- 0.034 | -2416772.436 +/- 1721.311 | 982.704 +/- 1.841 | 15.0 | 2.799 | 0.0 | the completed branch trajectory supports stable protein-level diagnostics and persistent proximity/contact sampling, but this does not validate entry inhibition, affinity, or druggability |
...
| 5E04-apo-protein | receptor-only control | running production on live SSH inspection, but archived protein-only analysis files through approximately 46.7 ns were already recovered locally in `md_report/status_refresh_20260525_062421_live_ssh/` and used here as a provisional structural-control layer | partially separates ligand-associated behavior from receptor-intrinsic fluctuations in the 5E04 branch, although the receptor-only trajectory is not yet complete |
| 9P3Y-apo-protein | receptor-only control | queued on live SSH inspection (`status.json`/`run.log` mtime 2026-05-25 07:25:26 -0600) with no production artifacts yet | needed to complete receptor-only baseline for 9P3Y branch comparison |
...
The current molecular-dynamics publication bundle, including per-system PNG, CSV, histogram panels, uniform contact-panel composites, typed granisetron interaction summaries, summary statistics, and conservative auto-interpretation, is archived in `md_report/publication_report_20260525_084126/`. A live-SSH apo/control status refresh for the present manuscript update, including archived remote `5E04-apo` protein-only analysis files and updated apo-job provenance, is archived separately in `md_report/status_refresh_20260525_062421_live_ssh/`.
