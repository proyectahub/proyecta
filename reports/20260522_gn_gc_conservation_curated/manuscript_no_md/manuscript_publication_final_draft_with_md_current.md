...
- Molecular dynamics evidence is currently incomplete: paroxetine has an analyzable 43.25 ns trajectory, whereas the full 50 ns candidate-control panel remains pending.
...
### Figure 6. Molecular dynamics results available to date
![Molecular dynamics results available to date](composite_figures/Figure_6_md_current_results.png)

Figure 6. Available paroxetine MD diagnostics. (A) Protein-ligand contact profile and residue heatmap. (B) Functional trajectory diagnostics exported by the portal. (C) Replotted RMSD, ligand RMSD, radius of gyration and H-bond traces with unit-corrected time axes. (D) Basic trajectory quality plots. These data are computational stability descriptors only and remain incomplete relative to the planned 50 ns endpoint.
